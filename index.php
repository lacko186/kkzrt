...
function initMap() {
    map = new google.maps.Map(document.getElementById('map'), {
        center: { lat: 46.3660, lng: 17.7950 }, //kaposvár
        zoom: 13
    });
    
    // After map is initialized, load GTFS data
    initializeGTFSData();
}

// Function to process and display GTFS data on the map
function processAndDisplayGTFS(gtfsData) {
    ...
    Object.entries(shapeGroups).forEach(([shapeId, points]) => {
        ...
        [points[0], points[points.length - 1]].forEach(point => {
            const marker = new google.maps.Marker({
                position: point,
                map: map,
                title: `Shape ID: ${shapeId}`
            });

            // Create info window
            const infoWindow = new google.maps.InfoWindow({
                content: `<div>Shape ID: ${shapeId}<br>Sequence: ${point.sequence}</div>`
            });
...
function initializeGTFSData() {
    fetch('gtfs_data.json')
        .then(response => {
            if (!response.ok) {
